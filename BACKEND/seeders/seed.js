...
async function seed() {
  ...
  try {
    ...
    `);
    console.log("Product portions seeded.");
    ...
    `);
    console.log("Modifier portions seeded.");
    ...
    await conn.query(`
      INSERT INTO cart_items (cart_item_id, cart_id, product_id, product_portion_id, quantity, price, is_deleted, created_by, updated_by) VALUES
        -- Rahul: Galaxy S24 Ultra 256GB (product=3, pp_id=6)
        (1, 1, 3, 6, 1, 124999.00, 0, 2, 2),
        -- Rahul: Redmi Note 13 Pro 256GB (product=5, pp_id=11)
        (2, 1, 5, 11, 1, 22999.00, 0, 2, 2),
        -- Rahul: Mi Smart Band 8 - NO portion, NO modifier (product=13, pp_id=NULL)
        (3, 1, 13, NULL, 1, 3499.00, 0, 2, 2),
        -- Priya: iPhone 15 Pro Max 256GB (product=1, pp_id=1)
        (4, 2, 1, 1, 1, 144999.00, 0, 3, 3),
        -- Priya: boAt Rockerz 450 - NO portion, NO modifier (product=14, pp_id=NULL)
        (5, 2, 14, NULL, 1, 1999.00, 0, 3, 3)
    `);

    // Cart item modifiers (which color/variant each cart item selected)
    // Note: cart items 3 and 5 (Mi Band, boAt) have no modifiers - no rows needed
    await conn.query(`
      INSERT INTO cart_item_modifiers (cart_item_modifier_id, cart_item_id, modifier_portion_id, additional_price, is_deleted, created_by, updated_by) VALUES
        -- Cart item 1 (S24 Ultra 256GB): Titanium Gray (mp_id=7)
        (1, 1, 7, 0.00, 0, 2, 2),
        -- Cart item 2 (Redmi Note 13 Pro 256GB): 12GB RAM (mp_id=14)
        (2, 2, 14, 2000.00, 0, 2, 2),
        -- Cart item 4 (iPhone 15 Pro Max 256GB): Natural Titanium (mp_id=3)
        (3, 4, 3, 0.00, 0, 3, 3)
    `);
    console.log("Carts and cart item modifiers seeded.");
    ...
    await conn.query(`
      INSERT INTO order_items (order_item_id, order_id, product_id, product_portion_id, product_name, portion_value, quantity, price, discount, tax, total, is_deleted, created_by, updated_by) VALUES
        -- Order 1: Rahul bought S24 Ultra 256GB + Redmi Note 13 Pro 256GB + Mi Smart Band 8
        (1, 1, 3, 6, 'Samsung Galaxy S24 Ultra', '256 GB', 1, 124999.00, 5000.00, 6500.00, 126499.00, 0, 2, 2),
        (2, 1, 5, 11, 'Redmi Note 13 Pro', '256 GB', 1, 22999.00, 5000.00, 500.00, 18499.00, 0, 2, 2),
        (3, 1, 13, NULL, 'Mi Smart Band 8', NULL, 1, 3499.00, 0.00, 180.00, 3679.00, 0, 2, 2),
        -- Order 2: Priya bought iPhone 15 Pro Max 512GB + boAt Rockerz 450
        (4, 2, 1, 2, 'Apple iPhone 15 Pro Max', '512 GB', 1, 164999.00, 5000.00, 7500.00, 167499.00, 0, 3, 3),
        (5, 2, 14, NULL, 'boAt Rockerz 450', NULL, 1, 1999.00, 0.00, 100.00, 2099.00, 0, 3, 3)
    `);

    // Order item modifiers (snapshot of selected modifiers at time of purchase)
    // Note: order items 3 and 5 (Mi Band, boAt) have no modifiers - no rows needed
    await conn.query(`
      INSERT INTO order_item_modifiers (order_item_modifier_id, order_item_id, modifier_portion_id, modifier_name, modifier_value, additional_price, is_deleted, created_by, updated_by) VALUES
        -- Order item 1 (S24 Ultra 256GB): Titanium Black (mp_id=8)
        (1, 1, 8, 'Color', 'Titanium Black', 0.00, 0, 2, 2),
        -- Order item 2 (Redmi Note 13 Pro 256GB): 8GB RAM (mp_id=13)
        (2, 2, 13, 'RAM', '8 GB', 0.00, 0, 2, 2),
        -- Order item 4 (iPhone 15 Pro Max 512GB): Blue (mp_id=5)
        (3, 4, 5, 'Color', 'Blue', 0.00, 0, 3, 3)
    `);
    console.log("Orders and order item modifiers seeded.");
    ...
    const [counts] = await conn.query(`
      SELECT
        (SELECT COUNT(*) FROM user_master) AS users,
        (SELECT COUNT(*) FROM user_addresses) AS addresses,
        (SELECT COUNT(*) FROM category_master) AS categories,
        (SELECT COUNT(*) FROM product_master) AS products,
        (SELECT COUNT(*) FROM product_categories) AS product_categories,
        (SELECT COUNT(*) FROM portion_master) AS portions,
        (SELECT COUNT(*) FROM product_portion) AS product_portions,
        (SELECT COUNT(*) FROM modifier_master) AS modifiers,
        (SELECT COUNT(*) FROM modifier_portion) AS modifier_portions,
        (SELECT COUNT(*) FROM cart_master) AS carts,
        (SELECT COUNT(*) FROM cart_items) AS cart_items,
        (SELECT COUNT(*) FROM cart_item_modifiers) AS cart_item_modifiers,
        (SELECT COUNT(*) FROM order_master) AS orders,
        (SELECT COUNT(*) FROM order_items) AS order_items,
        (SELECT COUNT(*) FROM order_item_modifiers) AS order_item_modifiers,
        (SELECT COUNT(*) FROM offer_master) AS offers,
        (SELECT COUNT(*) FROM offer_usage) AS offer_usages,
        (SELECT COUNT(*) FROM product_reviews) AS reviews,
        (SELECT COUNT(*) FROM payment_master) AS payments
    `);
    console.log("\nRecord counts:");
